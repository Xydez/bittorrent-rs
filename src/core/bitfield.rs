...
impl Bitfield {
	...
	pub fn from_bytes_length(data: &[u8], length: usize) -> Bitfield {
		assert!(
			data.len() * 8 > length && data.len() * 8 - length < 8,
			"Length out of bounds (data.len() * 8 > length && data.len() * 8 - length < 8)"
		);
...
mod tests {
	...
	fn test_create() {
		Bitfield::new(42);
		Bitfield::from_bytes(&[0b01010101, 0b10101010]);
	}

	#[test]
	fn test_len() {
		assert_eq!(Bitfield::new(42).len(), 42);
		assert_eq!(Bitfield::from_bytes(&[0b01010101, 0b10101010]).len(), 16);
		assert_eq!(
			Bitfield::from_bytes_length(&[0b01010101, 0b10101010], 12).len(),
			12
		);
	}
	...
	#[test]
	fn test_get() {
		let bitfield = Bitfield::from_bytes_length(&[0b01010101, 0b10101010], 12);
