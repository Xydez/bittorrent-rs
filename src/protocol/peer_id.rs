...
impl TryFrom<[u8; 20]> for PeerId {
    ...
    fn try_from(value: [u8; 20]) -> Result<Self> {
        ...
        if value[0] == b'-'
            ...
            let name = NAMES.get(&name_short).map(|v| v.0.to_string()).unwrap_or_else(|| String::from_utf8(name_short.to_vec()).unwrap());
            ...
            */
            ...
            let version = match NAMES
                .get(&name_short)
                .ok_or_else(|| ParseError::UnknownClient(
                    String::from_utf8(name_short.to_vec()).unwrap()
                ))?.1 {
