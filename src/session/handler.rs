...
async fn handle_torrent_event(
	...
	match event {
		...
		TorrentEvent::PeerEvent(_peer, PeerEvent::BlockReceived(piece, block)) => {
			handle_block_received(&prefix, session, torrent_id, *piece, *block).await;
		}
		TorrentEvent::PieceEvent(piece, event) => match event {
			PieceEvent::Downloaded(data) => {
				handle_piece_downloaded(&prefix, session, *piece, torrent_id, data.clone()).await;
			}
			PieceEvent::Verified(data) => {
				handle_piece_verified(&prefix, session, torrent_id, *piece, data.clone()).await;
			}
			PieceEvent::Done => {
...
async fn handle_piece_downloaded(
	prefix: &str,
	session: &Session,
	piece: u32,
	torrent_id: TorrentId,
	...
	if state != piece::State::Pending {
		log::warn!("Piece {piece} downloaded with non-pending state (state = {state:?}");
		return;
	}

	tokio::spawn(async move {
		let _permit = semaphore.acquire_owned().await.unwrap();
		torrent.lock().await.state_mut().pieces[piece as usize].state = piece::State::Verifying;
		...
		if intact {
			...
			torrent.lock().await.state_mut().pieces[piece as usize].state = piece::State::Pending;
		}
	});
}
...
async fn handle_piece_verified(
	prefix: &str,
	session: &Session,
	torrent_id: TorrentId,
	piece: u32,
	...
	assert!(torrent.lock().await.state().pieces[piece as usize].state == piece::State::Verifying);

	// TODO: (Maybe) push the data onto an io writer queue or similar?
	tokio::spawn(async move {
		torrent.lock().await.state_mut().pieces[piece as usize].state = piece::State::Writing;
